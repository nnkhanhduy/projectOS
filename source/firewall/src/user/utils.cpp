...
// Đọc file firewall_configs.json, parse JSON và nạp vào rules_map
...
// Đọc ioc_dns.json và nạp vào dns_map
int load_dns_ioc_map(struct firewall_bpf *skel, const char *json_path)
{
    ...
    if (!root) {
        std::cerr << "[ERROR] JSON parse error: " << cJSON_GetErrorPtr() << std::endl;
        return -1;
    }

    cJSON *arr = cJSON_GetObjectItem(root, "blocked_domains");
    if (!arr || !cJSON_IsArray(arr)) {
        std::cerr << "[ERROR] 'blocked_domains' not found or not array\n";
        cJSON_Delete(root);
        return -1;
    }

    // 3. Get map FD
    int map_fd = bpf_map__fd(skel->maps.dns_map);
    if (map_fd < 0) {
        std::cerr << "[ERROR] Failed to get dns_map fd\n";
        cJSON_Delete(root);
        return -1;
    }
    ...
    cJSON_ArrayForEach(item, arr) {
        ...
        const char *domain = item->valuestring;
        if (strlen(domain) == 0 || strlen(domain) >= MAX_DNS_NAME_LENGTH) {
            std::cerr << "[SKIP] Invalid domain: " << domain << std::endl;
            failed++;
            continue;
        }
        ...
        if (bpf_map__update_elem(skel->maps.dns_map,
                                 ...
                                 BPF_ANY) != 0)
        {
            std::cerr << "[FAIL] Insert failed: " << domain
                      << " (" << strerror(errno) << ")\n";
            failed++;
        }
        else {
            std::cerr << "[OK] Loaded DNS IOC: " << domain << std::endl;
            loaded++;
        }
    }

    // Summary
    std::cerr << "=== DNS IOC LOADED ===\n";
    std::cerr << "   Success: " << loaded << "\n";
...
// Đọc file text danh sách IP độc hại và nạp vào ioc_ip_map
bool load_ioc_ip_into_kernel_map(struct firewall_bpf *skel, const std::string &path)
{
    ...
    while (std::getline(infile, line)) {
        ...
        if (line.find(':') != std::string::npos) {
            ...
            if (inet_pton(AF_INET6, line.c_str(), lpm_key.data) != 1) {
            ...
            if (inet_pton(AF_INET, line.c_str(), lpm_key.data) != 1) {
